...
**Primary Dependencies**: Existing JWT auth (004), inventory/fulfillment services (011), admin order endpoints (010/012). No new packages, no schema changes (role is already a string column); role seed data is delivered by explicit EF Core migration data.
...
- **DD-005**: `AccountModelConstants.Roles.Supplier = "supplier"`; explicit EF Core migration seed data adds `supplier`/`supplier` (role `supplier`). The seed remains idempotent per user id, so existing databases gain the supplier without touching other users (spec edge case). Application startup must not create or update the supplier user implicitly.
...
- Migration seed data coverage including the new supplier user after `Database.MigrateAsync`.
